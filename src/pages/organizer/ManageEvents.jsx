...
import { HiPlus, HiPencil, HiEye, HiUpload, HiBan } from 'react-icons/hi';
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            {events.length === 0 ? (
                <div className="card p-12 text-center">
                    ...
                    <p className="text-gray-500 mt-2 mb-6">Create your first event to get started</p>
                                ...
                                <tr key={event._id} className="hover:bg-gray-50">
                                    <td className="px-4 py-4">
                                        <div>
                                            ...
                                            {event.artist && <p className="text-sm text-gray-500">{event.artist}</p>}
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-4 py-4">
                                        <div className="flex items-center justify-end gap-2">
                                            <Link to={`/events/${event._id}`} className="p-2 text-gray-500 hover:text-primary-600" title="View">
                                                <HiEye />
                                            </Link>
                                            {event.status === 'draft' && (
                                                <>
                                                    <Link to={`/organizer/edit/${event._id}`} className="p-2 text-gray-500 hover:text-primary-600" title="Edit">
                                                        <HiPencil />
                                                    </Link>
                                                    <button
                                                        ...
                                                        title={!isApprovedOrganizer ? 'Awaiting approval' : !hasPaystack ? 'Setup paystack first' : 'Publish'}
                                                    >
                                                        <HiUpload />
                                                    </button>
                                                </>
                                            ...
                                            {event.status === 'published' && (
                                                <button onClick={() => handleCancel(event._id)} className="p-2 text-red-500 hover:text-red-700" title="Cancel">
                                                    <HiBan />
                                                </button>
                                            )}
                                        </div>
