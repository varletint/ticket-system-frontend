...
const Reconciliation = () => {
  ...
  const runReconciliation = async () => {
    if (
      !confirm(
        "Run full reconciliation? This will fix event ticket counts and revenue totals."
      )
    )
      return;
    try {
      ...
      alert(
        `Reconciliation complete: ${res.data.data.eventsFixed} events fixed in ${res.data.data.duration}`
      );
      fetchData();
    } catch (error) {
          ...
          <div className='space-y-3'>
            ...
            <div className='flex justify-between'>
              ...
              <span className='font-bold'>
                ₦{(summary?.orders?.revenue || 0).toLocaleString()}
              </span>
            </div>
            <div className='flex justify-between'>
              <span className='text-gray-500'>Expected Tickets</span>
              <span className='font-bold'>
                {summary?.orders?.ticketsExpected || 0}
              </span>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          <div className='space-y-3'>
            ...
            <div className='flex justify-between'>
              ...
              <span className='font-bold'>
                ₦{(summary?.transactions?.net || 0).toLocaleString()}
              </span>
            </div>
            <div className='flex justify-between'>
              <span className='text-gray-500'>Discrepancy</span>
        ...
        <div className='grid grid-cols-3 gap-4'>
          <div
            className={`p-4 rounded-lg text-center ${
              summary?.health?.ticketsHealthy ? "bg-green-50" : "bg-red-50"
            }`}>
          ...
          <div
            className={`p-4 rounded-lg text-center ${
              summary?.health?.revenueHealthy ? "bg-green-50" : "bg-red-50"
            }`}>
          ...
          <div
            className={`p-4 rounded-lg text-center ${
              summary?.health?.transactionsHealthy ? "bg-green-50" : "bg-red-50"
            }`}>
