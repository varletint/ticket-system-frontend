...
const DisputeManagement = () => {
  ...
  return (
    <div className='max-w-7xl mx-auto px-4 py-8'>
      <h1 className='text-2xl font-bold text-text mb-6'>Dispute Management</h1>

      {/* Stats Cards */}
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-6'>
        <div className='card p-2 border-l-4 border-red-500'>
          <div className='flex items-center gap-3'>
            <HiExclamationCircle className='text-2xl text-red-500' />
            <div>
              <p className='text-xs text-text/70'>Open</p>
              <p className='text-2xl font-bold'>
                {stats?.summary?.openCount || 0}
              </p>
            </div>
          </div>
        </div>
        <div className='card p-2 border-l-4 border-blue-500'>
          <div className='flex items-center gap-3'>
            <HiClock className='text-2xl text-blue-500' />
            <div>
              <p className='text-xs text-text/70'>Investigating</p>
              <p className='text-2xl font-bold'>
                {stats?.summary?.investigatingCount || 0}
              </p>
            </div>
          </div>
        </div>
        <div className='card p-2 border-l-4 border-green-500'>
          <div className='flex items-center gap-3'>
            <HiCheckCircle className='text-2xl text-green-500' />
            <div>
              <p className='text-xs text-text/70'>Resolved</p>
              <p className='text-2xl font-bold'>
                {stats?.summary?.resolvedCount || 0}
              </p>
            </div>
          </div>
        </div>
        <div className='card p-2 border-l-4 border-text/20'>
          <div className='flex items-center gap-3'>
            <HiXCircle className='text-2xl text-text/70' />
            <div>
              <p className='text-xs text-text/70'>Rejected</p>
      ...
      {stats?.urgentDisputes > 0 && (
        <div className='bg-red-50 border border-red-200 text-red-700 px-4 py-3  mb-6 flex items-center gap-2'>
          ...
          <span className='font-medium'>
            {stats.urgentDisputes} urgent dispute(s) require immediate
            attention!
          </span>
        </div>
      )}
      ...
      <div className='flex gap-2 mb-6'>
        {["open", "investigating", "pending_user", "resolved", "rejected"].map(
          (status) => (
            <button
              key={status}
      ...
      <div className='card overflow-hidden'>
        <div className='overflow-x-auto'>
          <table className='w-full'>
            <thead className='bg-text/20'>
              <tr>
                <th className='px-4 py-3 text-left text-xs font-medium text-text/70 uppercase'>
                  Dispute #
                </th>
                <th className='px-4 py-3 text-left text-xs font-medium text-text/70 uppercase'>
                  Type
                ...
                <th className='px-4 py-3 text-right text-xs font-medium text-text/70 uppercase'>
                  Amount
                </th>
                <th className='px-4 py-3 text-center text-xs font-medium text-text/70 uppercase'>
                  Priority
                </th>
                <th className='px-4 py-3 text-center text-xs font-medium text-text/70 uppercase'>
                  Status
            ...
            <tbody className='divide-y divide-text/20'>
              {disputes.map((dispute) => (
                <tr
                  key={dispute._id}
                  className='hover:bg-text/20 text-text even:bg-text/5'>
                  <td className='px-4 py-3 text-sm font-mono text-nowrap'>
                    {dispute.disputeNumber}
                  </td>
                  <td className='px-4 py-3 text-sm capitalize text-nowrap'>
                  ...
                  <td className='px-4 py-3 text-sm text-nowrap'>
                    {dispute.event?.title || "N/A"}
                  </td>
                  <td className='px-4 py-3 text-sm text-right font-medium text-nowrap'>
        ...
        {disputes.length === 0 && (
          <div className='p-8 text-center text-text/70'>No disputes found</div>
        )}
      </div>
      ...
      {selectedDispute && (
        <div className='fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50'>
          <div className='bg-surface  max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6'>
            <div className='flex justify-between items-start mb-4'>
              ...
              <button
                ...
                className='text-text/70 hover:text-text/80'>
                ✕
              </button>
            </div>
            ...
            <div className='grid grid-cols-2 gap-4 mb-4'>
              <div>
                <p className='text-text/70 text-sm'>Status</p>
                {getStatusBadge(selectedDispute.status)}
              </div>
              <div>
                <p className='text-text/70 text-sm'>Priority</p>
                {getPriorityBadge(selectedDispute.priority)}
              </div>
              <div>
                <p className='text-text/70 text-sm'>Type</p>
                <p className='capitalize'>
                  {selectedDispute.type?.replace("_", " ")}
                </p>
              </div>
              <div>
                <p className='text-text/70 text-sm'>Amount</p>
                <p className='font-bold'>
                  {new Intl.NumberFormat("en-NG", {
                    style: "currency",
                    currency: "NGN",
                  }).format(selectedDispute.amount) || 0}
                </p>
              </div>
            </div>

            <div className='mb-4'>
              <p className='text-text/70 text-sm mb-1'>Description</p>
              <p className='bg-text/20 p-3 '>{selectedDispute.description}</p>
            </div>

            {/* Status Update Buttons */}
            {!["resolved", "rejected"].includes(selectedDispute.status) && (
              <div className='mb-4'>
                <p className='text-text/70 text-sm mb-2'>Update Status</p>
                <div className='flex gap-2'>
                  {selectedDispute.status === "open" && (
                    <button
                      onClick={() =>
                        handleUpdateStatus(selectedDispute._id, "investigating")
                      }
                      disabled={processing}
                  ...
                  {selectedDispute.status === "investigating" && (
                    <button
                      onClick={() =>
                        handleUpdateStatus(selectedDispute._id, "pending_user")
                      }
                      disabled={processing}
                      className='btn text-sm'>
                      Request User Info
                ...
                <div className='space-y-3'>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setResolution({ ...resolution, type: e.target.value })
                      }
              ...
              selectedDispute.resolution && (
                <div className='border-t pt-4'>
                  ...
                  <p>
                    ...
                    {selectedDispute.resolution.refundAmount?.toLocaleString()}
                  </p>
                  <p>
