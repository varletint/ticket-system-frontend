                    ...
                    <div className="hidden md:flex items-center gap-6">
                        ...
                        {user ? (
                            <>
                                ...
                                {(isOrganizer || isAdmin) && (
                                    <Link to="/organizer" className="text-gray-600 hover:text-primary-600 transition-colors">
                                        Organizer
                                    </Link>
                                )}

                                {(isValidator || isAdmin) && (
                                    <Link to="/validator" className="text-gray-600 hover:text-primary-600 transition-colors">
                                        Validator
                                    </Link>
                                )}
                ...
                {isMenuOpen && (
                    <div className="md:hidden py-4 border-t border-gray-100 animate-fade-in">
                        <div className="flex flex-col gap-3">
                            <Link to="/events" className="px-3 py-2 text-gray-600 hover:bg-gray-50 rounded-lg">
                                Events
                            </Link>

                            {user ? (
                                <>
                                    <Link to="/my-tickets" className="px-3 py-2 text-gray-600 hover:bg-gray-50 rounded-lg">
                                        My Tickets
                                    </Link>

                                    {(isOrganizer || isAdmin) && (
                                        <Link to="/organizer" className="px-3 py-2 text-gray-600 hover:bg-gray-50 rounded-lg">
                                            Organizer Dashboard
                                        </Link>
                                    )}

                                    {(isValidator || isAdmin) && (
                                        <Link to="/validator" className="px-3 py-2 text-gray-600 hover:bg-gray-50 rounded-lg">
                                            Validator Dashboard
                                        </Link>
                                    )}

                                    {isAdmin && (
                                        <Link to="/admin" className="px-3 py-2 text-gray-600 hover:bg-gray-50 rounded-lg">
                                            Admin Dashboard
                                        </Link>
                                    )}

                                    <hr className="my-2" />
                                    <Link to="/profile" className="px-3 py-2 text-gray-600 hover:bg-gray-50 rounded-lg">
                                        My Profile
                                    </Link>
